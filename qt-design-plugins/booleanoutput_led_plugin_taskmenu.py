...
class LEDMenuEntry(QPyDesignerTaskMenuExtension):
    """Task menu entry for an LED"""

    def __init__(self, widget, parent):
        super(LEDMenuEntry, self).__init__(parent)
        self.widget = widget
    ...
    def updateSettings(self):
        """Update the settings"""
        dialog = LEDDialog(self.widget)
        dialog.exec_()


class LEDTaskMenuFactory(QExtensionFactory):
    """TaskMenuFactory for LED"""

    def __init__(self, parent=None):
        super(LEDTaskMenuFactory, self).__init__(parent)
    ...
    def createExtension(self, obj, iid, parent):  # pylint: disable=no-self-use
        ...
        if isinstance(obj, LED):
            return LEDMenuEntry(obj, parent)

        return None


class LEDDialog(QDialog):
    """The settings dialog for the LED"""
    ...
    def __init__(self, widget, parent=None):
        ...
        #- initial state
        self.previewWidget.setState(widget.getState())
        self.initial_state.setChecked(widget.getState())

        # Connect signals
        self.color_scheme.currentIndexChanged[str].connect(self.previewWidget.setColorScheme)
        self.initial_state.toggled.connect(self.previewWidget.setState)
    ...
    def updateWidget(self):
        ...
        if formWindow:
            ...
            formWindow.cursor().setProperty(
                "state", QVariant(self.initial_state.isChecked()))
        self.accept()
